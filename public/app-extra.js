...
function searchSantriPick(prefix, q) {
  const box = $(prefix + "SearchResults"),
    hidden = $(prefix + "Santri");
  if (!q || q.length < 1) {
    box.innerHTML = "";
    hidden.value = "";
    return;
  }
  const results = _allSantriList
    .filter((s) => s.nama.toLowerCase().includes(q.toLowerCase()))
    .slice(0, 8);
  box.innerHTML = results
    .map(
      (s) =>
        `<div onclick="pickSantri('${prefix}',${s.id},'${s.nama.replace(/'/g, "\\'")}')">${s.nama}</div>`,
    )
    .join("");
}
function pickSantri(prefix, id, nama) {
  $(prefix + "Santri").value = id;
  $(prefix + "Search").value = nama;
  $(prefix + "SearchResults").innerHTML = "";
}

// -- CATATAN GURU --
async function loadCatatanGuru() {
  const [santri, catatan] = await Promise.all([
    api("/api/santri"),
    api("/api/catatan-guru"),
  ]);
  _allSantriList = santri;
  const today = new Date(Date.now() + 7 * 3600000).toISOString().slice(0, 10);
  $("main").innerHTML =
    `<div class="page-header fade-up"><h2> Catatan Guru</h2></div>
        ...
        catatan.length
          ? catatan
              .map(
                (
                  c,
                ) => `<tr><td>${c.tanggal}</td><td>${c.santri_nama || "-"}</td><td>${c.catatan}</td>
        <td><button class="btn btn-danger btn-sm" onclick="hapusCatatanGuru(${c.id})"></button></td></tr>`,
              )
              .join("")
...
async function simpanCatatanGuru() {
  const sid = parseInt($("cgSantri").value) || 0;
  if (!sid) return alert("Pilih santri terlebih dahulu");
  const cat = $("cgCatatan").value.trim(),
    tgl = $("cgTanggal").value;
  if (!cat) return alert("Catatan kosong");
  await api("/api/catatan-guru", {
    method: "POST",
    body: JSON.stringify({ santri_id: sid, catatan: cat, tanggal: tgl }),
  });
  loadCatatanGuru();
}
async function hapusCatatanGuru(id) {
  if (!confirm("Hapus?")) return;
  await api("/api/catatan-guru/" + id, { method: "DELETE" });
  loadCatatanGuru();
}
...
async function loadPelanggaran() {
  const [santri, pel] = await Promise.all([
    api("/api/santri"),
    api("/api/pelanggaran"),
  ]);
  _allSantriList = santri;
  const today = new Date(Date.now() + 7 * 3600000).toISOString().slice(0, 10);
  const tabList = _pelTab === "list" ? "background:var(--p);color:#fff" : "",
    tabRekap = _pelTab === "rekap" ? "background:var(--p);color:#fff" : "";
  $("main").innerHTML =
    `<div class="page-header fade-up"><h2>⚠️ Pelanggaran</h2></div>
  ...
  const totalPel = pel.length,
    sudahT = pel.filter((p) => p.status_takzir === "sudah").length,
    belumT = totalPel - sudahT;
  const totalDenda = pel
    .filter((p) => p.status_takzir === "sudah")
    .reduce((s, p) => s + (p.denda || 0), 0);
  $("main").innerHTML += `
        ...
        pel.length
          ? pel
              .map((p) => {
                ...
                const dendaInfo = p.denda
                  ? '<br><span style="font-size:.68rem;color:#dc2626">Denda: ' +
                    fmtRp(p.denda) +
                    "</span>"
                  : "";
                const takzirInfo = p.jenis_takzir
                  ? '<br><span style="font-size:.68rem;color:var(--t3)">' +
                    p.jenis_takzir +
                    "</span>"
                  : "";
...
function showTakzirModal(
  ...
  $("modal").innerHTML =
    `<h3><i class="ri-shield-check-line"></i> Takzir: ${santriNama}</h3>
...
async function simpanTakzir(id, status) {
  const jenisTakzir =
    status === "sudah" ? $("tzJenis")?.value?.trim() || "" : "";
  const isDenda = status === "sudah" ? $("tzIsDenda")?.checked || false : false;
  const denda = isDenda ? parseInt($("tzDenda")?.value || 0) : 0;
  if (status === "sudah" && !jenisTakzir)
    return alert("Jenis takziran wajib diisi");
  await api("/api/pelanggaran/" + id + "/takzir", {
    method: "PUT",
    body: JSON.stringify({
      status_takzir: status,
      jenis_takzir: jenisTakzir,
      denda,
    }),
  });
  hideModal();
  toast(status === "sudah" ? "✅ Takzir dicatat" : "Takzir dibatalkan");
  loadPelanggaran();
}

async function simpanPelanggaran() {
  const sid = parseInt($("plgSantri").value) || 0;
  if (!sid) return alert("Pilih santri terlebih dahulu");
  const jenis = $("plgJenis").value.trim(),
    desk = $("plgDeskripsi").value.trim(),
    poin = parseInt($("plgPoin").value) || 0,
    tgl = $("plgTanggal").value;
  if (!jenis) return alert("Jenis wajib");
...
async function hapusPelanggaran(id) {
  if (!confirm("Hapus?")) return;
  await api("/api/pelanggaran/" + id, { method: "DELETE" });
  loadPelanggaran();
}
...
const FEATURE_GROUPS = [
  ...
  {
    section: "Sekolah & Diniyyah",
    items: [
      {
        key: "kelas_sekolah",
        label: "Kelas & Absen Sekolah",
        icon: "ri-school-line",
      },
      {
        key: "absen_sekolah",
      ...
      {
        key: "madrasah_diniyyah",
        label: "Madrasah Diniyyah",
        icon: "ri-book-2-line",
      },
    ],
  },
  {
    ...
    items: [
      {
        key: "kedisiplinan",
        label: "Catatan, Pelanggaran & Perizinan",
  ...
  },
  {
    ...
    items: [
      ...
      {
        key: "sangu",
        label: "Sangu Santri (Kantin)",
        icon: "ri-wallet-3-line",
      },
    ],
  },
];
const FEATURE_LIST = FEATURE_GROUPS.flatMap(function (g) {
  return g.items;
});
...
function featureGrid(sel, pfx) {
  ...
  var html =
    '<div class="fg"><label>Fitur Aktif</label><div style="display:flex;gap:.4rem;margin-bottom:.5rem"><button type="button" class="btn btn-outline btn-sm" onclick="document.querySelectorAll(\'input[name=' +
    pfx +
    "]').forEach(function(c){c.checked=true;c.closest('label').style.background='var(--greenbg)';c.closest('label').style.borderColor='var(--green)'})\" ><i class=\"ri-checkbox-circle-line\"></i> Semua</button><button type=\"button\" class=\"btn btn-outline btn-sm\" onclick=\"document.querySelectorAll('input[name=" +
    pfx +
    "]').forEach(function(c){c.checked=false;c.closest('label').style.background='#f8fafc';c.closest('label').style.borderColor='var(--border)'})\" ><i class=\"ri-close-circle-line\"></i> Hapus</button></div>";
  FEATURE_GROUPS.forEach(function (g) {
    html +=
      '<div style="font-size:.72rem;font-weight:700;color:var(--t3);text-transform:uppercase;letter-spacing:.05em;margin:.6rem 0 .3rem;padding-left:.1rem">' +
      g.section +
      "</div>";
    html +=
    ...
    g.items.forEach(function (f) {
      ...
      html +=
        '<label style="display:flex;align-items:center;gap:.4rem;padding:.45rem .6rem;background:' +
        (on ? "var(--greenbg)" : "#f8fafc") +
        ";border:1.5px solid " +
        (on ? "var(--green)" : "var(--border)") +
        ";border-radius:10px;cursor:pointer;font-size:.78rem;font-weight:500;transition:.2s\" onclick=\"setTimeout(function(){var c=this.querySelector('input');this.style.background=c.checked?'var(--greenbg)':'#f8fafc';this.style.borderColor=c.checked?'var(--green)':'var(--border)'}.bind(this),10)\"><input type=\"checkbox\" name=\"" +
        pfx +
        '" value="' +
        f.key +
        '" ' +
        (on ? "checked" : "") +
        ' style="accent-color:var(--green);width:16px;height:16px"><i class="' +
        f.icon +
        '" style="font-size:.9rem;color:var(--p)"></i>' +
        f.label +
        "</label>";
    });
    html += "</div>";
...
function getFeats(pfx) {
  return [...document.querySelectorAll("input[name=" + pfx + "]:checked")].map(
    function (c) {
      return c.value;
    },
  );
...
async function loadTenants() {
  ...
  const t = _tenantList;
  $("main").innerHTML =
    `<div class="page-header au"><h2><i class="ri-building-2-line"></i> Kelola Tenant</h2>
    ...
    <div class="card au"><div class="table-wrap"><table>
      ...
      ${t
        .map((x, _tidx) => {
          ...
          const statusBadge =
            x.status === "active" && !isExpired
              ? '<span class="badge-h">Active</span>'
              : isExpired
                ? '<span class="badge-a">Expired</span>'
                : '<span class="badge-i">Suspended</span>';
          const feats = parseFeatures(x.features);
          const featBadge = feats.length
            ? `<span style="font-size:.68rem;color:var(--p);font-weight:600">${feats.length}/${FEATURE_LIST.length}</span>`
            : '<span style="font-size:.68rem;color:var(--green);font-weight:600">Semua</span>';
          const pktBadge =
            x.paket_saas === "premium"
              ? '<span class="badge-h">Premium</span>'
              : x.paket_saas === "standart"
                ? '<span class="badge-p">Standart</span>'
                : '<span class="badge-i">Trial</span>';
          const sngBadge = x.fitur_sangu
            ? '<span class="badge-h">Aktif</span>'
            : '<span class="badge-a">Nonaktif</span>';
...
function showSaaSSettingsTenant(idx) {
  ...
  $("modal").innerHTML =
    `<h3><i class="ri-vip-crown-line"></i> Atur SaaS: ${x.nama}</h3>
...
async function saveSaaSSettingsTenant(id) {
  ...
  await api("/api/saas/tenants/" + id + "/fitur", {
    method: "PUT",
    body: JSON.stringify(body),
  });
  hideModal();
  toast("Berhasil update SaaS tenant");
  loadTenants();
}
function showAddTenant() {
  $("modal").innerHTML =
    `<h3><i class="ri-building-2-line"></i> Tambah Tenant</h3>
...
async function saveTenant() {
  ...
  hideModal();
  toast(r.message || "Ditambahkan");
  loadTenants();
}
async function delTenant(id) {
  if (!confirm("Hapus tenant? SEMUA DATA terhapus!")) return;
  await api("/api/super/tenants/" + id, { method: "DELETE" });
  toast("Dihapus");
  loadTenants();
}

function showEditFeaturesIdx(idx) {
  const x = _tenantList[idx];
  if (!x) return;
  showEditFeatures(x.id, x.nama, x.features || "");
}
function showEditFeatures(id, nama, featStr) {
  const feats = parseFeatures(featStr);
  $("modal").innerHTML =
    `<h3><i class="ri-settings-3-line"></i> Fitur: ${nama}</h3>
...
function showExtendTenant(id, nama, currentExp) {
  $("modal").innerHTML =
    `<h3><i class="ri-time-line"></i> Perpanjang: ${nama}</h3>
...
async function doExtendTenant(id) {
  ...
  const r = await api("/api/super/tenants/" + id + "/extend", {
    method: "PUT",
    body: JSON.stringify({ months }),
  });
  hideModal();
  toast(r.message || "Diperpanjang");
  loadTenants();
}

// Toggle active/suspend
async function toggleTenant(id) {
  const r = await api("/api/super/tenants/" + id + "/toggle", {
    method: "PUT",
  });
  toast(r.message || "Status diubah");
  loadTenants();
}
...
async function doChangePassword() {
  ...
  try {
    const r = await api("/api/super/change-password", {
      method: "PUT",
      body: JSON.stringify({ old_password, new_password }),
    });
    hideModal();
    toast(r.message || "Password diubah");
  } catch (e) {
    toast(e.message);
  }
}
...
function showBroadcast() {
  $("modal").innerHTML =
    `<h3><i class="ri-megaphone-line"></i> Broadcast Pengumuman</h3>
...
async function loadSuperStats() {
  ...
  try {
    ...
    m.innerHTML =
      '<div class="card"><p style="color:red">Error loading stats</p></div>';
  }
}

// -- USERS & SETTINGS --
let _usersData = [];
let _usersFilter = "semua";
...
function renderUsers() {
  ...
  const roleLabels = {
    ...
    keamanan: "Keamanan",
    wali: "Wali Santri",
    merchant_admin: "Admin Kantin",
    kasir: "Kasir Kantin",
  };
  const roleIcons = {
    admin: "ri-shield-star-line",
    ustadz: "ri-user-heart-line",
    bendahara: "ri-money-dollar-circle-line",
    keamanan: "ri-shield-check-line",
    wali: "ri-parent-line",
    merchant_admin: "ri-store-2-line",
    kasir: "ri-shopping-cart-2-line",
  };
  const roleColors = {
    admin: "#3b82f6",
    ustadz: "#10b981",
  ...
  const counts = {};
  roleOrder.forEach(
    (r) => (counts[r] = list.filter((u) => u.role === r).length),
  );
...
async function bulkToggleWali(isActive) {
  const ids = [...document.querySelectorAll(".wali-cb:checked")].map((cb) =>
    parseInt(cb.value),
  );
  ...
  try {
    await api("/api/users/bulk-toggle", {
      method: "PUT",
      body: JSON.stringify({ ids, is_active: isActive }),
    });
    toast(
      `[OK] ${ids.length} akun wali ${isActive ? "diaktifkan" : "dinonaktifkan"}`,
    );
    loadUsers();
  } catch (e) {
...
async function showAddUser() {
  let merchants = [];
  try {
    merchants = await api("/api/sangu/merchants");
  } catch (e) {}

  $("modal").innerHTML =
    `<h3><i class="ri-user-add-line"></i> Tambah Pengguna</h3>
...
async function saveUser() {
  ...
  if (!b.username || !b.password || !b.nama)
...
async function loadSettings() {
  ...
  const pgProviders = [
    { v: "", l: "Tidak Aktif" },
    { v: "midtrans", l: "Midtrans" },
  ];
  const pgProviderOpts = pgProviders
    .map(
      (p) =>
        `<option value="${p.v}" ${(s.pg_provider || "") === p.v ? "selected" : ""}>${p.l}</option>`,
    )
    .join("");
  $("main").innerHTML =
      `<div class="page-header fade-up"><h2> Pengaturan</h2></div><div class="card fade-up"><h3><i class="ri-building-line"></i> Informasi Lembaga</h3><div class="fg"><label>Nama Aplikasi</label><input id="stN" value="${s.app_name || ""}"></div><div class="fg"><label>Alamat</label><input id="stA" value="${s.alamat_lembaga || ""}"></div><div class="fg"><label>Kepala</label><input id="stK" value="${s.kepala_nama || ""}"></div><div class="fg"><label>Kota</label><input id="stKo" value="${s.nama_kota || ""}"></div></div>
      <div class="card fade-up"><h3><i class="ri-calendar-todo-line"></i> Tahun Ajaran & Semester Aktif</h3>
        <p style="font-size:.78rem;color:var(--t3);margin-bottom:.8rem">Pengaturan ini akan digunakan secara default di seluruh menu penilaian, raport, dan jadwal.</p>
        <div style="display:flex;gap:.8rem;">
          <div class="fg" style="flex:1"><label>Tahun Ajaran</label><input id="stThnAjaran" value="${s.tahun_ajaran_aktif || "2026/2027"}" placeholder="Contoh: 2026/2027"></div>
          <div class="fg" style="flex:1"><label>Semester</label><select id="stSemesterAktif" style="padding:.65rem;border-radius:10px;border:1.5px solid var(--border);width:100%;font-size:.85rem">
      ...
      <div class="card fade-up"><h3><i class="ri-image-line"></i> Logo Lembaga</h3>
...
function togglePGFields() {
  const v = $("stPgProvider").value;
  $("pgFieldsArea").style.display = v ? "block" : "none";
}
async function saveSettings() {
  const appName = $("stN").value,
    alamat = $("stA").value,
    kepala = $("stK").value,
    kota = $("stKo").value;
  const rekBank = $("stRekBank")?.value || "",
    rekNomor = $("stRekNomor")?.value || "",
    rekNama = $("stRekNama")?.value || "",
    bendTelp = $("stBendTelp")?.value || "";
  ...
  const thnAjaran = $("stThnAjaran")?.value || "";
  const semesterAktif = $("stSemesterAktif")?.value || "";
  ...
  try {
    const body = {
      ...
      kepala_nama: kepala,
      nama_kota: kota,
      rekening_bank: rekBank,
      rekening_nomor: rekNomor,
      rekening_atas_nama: rekNama,
      bendahara_telp: bendTelp,
      psb_kuota: parseInt($("stPsbKuota")?.value) || 0,
      pg_provider: pgProvider,
      pg_server_key: pgServerKey,
      pg_client_key: pgClientKey,
      pg_is_production: pgIsProduction,
      pg_fee_percent: pgFeePercent,
      pg_fee_flat: pgFeeFlat,
      logo_base64: logoBase64,
      tahun_ajaran_aktif: thnAjaran,
      ...
      tagihan_bulan_mulai: $("stTagihanMulai")?.value || "",
      logo_base64: logoBase64,
    };
    ...
    if (verify.app_name !== appName) {
      ...
      alert(
        "Settings tidak tersimpan di database!\n\nYang dikirim: " +
          appName +
          "\nYang di DB: " +
          (verify.app_name || "kosong") +
    ...
    document.querySelector(".sb-sub").textContent =
      verify.alamat_lembaga || "Management System";
...
async function loadJadwalUmum() {
  const [jadwal, kelompok, users, kegiatan] = await Promise.all([
    api("/api/jadwal-umum"),
    api("/api/kelompok"),
    api("/api/users").catch(() => []),
    api("/api/kegiatan").catch(() => []),
  ]);
  ...
  jadwal.forEach((j) => {
    if (j.hari === "Setiap Hari") {
      hariList.forEach((h) => grouped[h].push(j));
    } else if (grouped[j.hari]) grouped[j.hari].push(j);
  });
  $("main").innerHTML =
    `<div class="page-header au"><h2><i class="ri-calendar-schedule-line"></i> Jadwal Kegiatan</h2>
    ...
    ${hariList
      ...
      ${items
        ...
        ${["admin", "superadmin"].includes(user.role) ? `<td><button class="btn btn-danger btn-sm" onclick="hapusJadwal(${j.id})"><i class="ri-delete-bin-line"></i></button></td>` : ""}</tr>`,
        )
        .join("")}
      </table></div></div>`;
      })
  ...
  window._kelompokList = kelompok;
  window._ustadzList = ustadzList;
  window._kegiatanList = kegiatan;
}
function showAddJadwal() {
  const kel = window._kelompokList || [];
  const ust = window._ustadzList || [];
  const keg = window._kegiatanList || [];
  const kegList = keg.length
    ? keg
    : [
        ...new Map(
          kel
            .filter((k) => k.kegiatan_nama)
            .map((k) => [
              k.kegiatan_id || k.kegiatan_nama,
              { id: k.kegiatan_id, nama: k.kegiatan_nama },
            ]),
        ).values(),
      ];
  const allHari = [
  ...
  $("modal").innerHTML =
    `<h3><i class="ri-calendar-schedule-line"></i> Tambah Jadwal</h3>
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:.4rem">
        ${allHari
          .map(
            (
              h,
            ) => `<label style="display:flex;align-items:center;gap:.35rem;padding:.4rem .6rem;background:#f8fafc;border:1.5px solid var(--border);border-radius:10px;cursor:pointer;font-size:.78rem;font-weight:500;transition:.2s"
      ...
      ${
        ust.length
          ? ust
              .map(
                (u) =>
                  `<option value="${u.username}">${u.nama} (${u.username})</option>`,
              )
              .join("")
...
function filterJadwalKelompok() {
  const sel = $("jKegiatan").value;
  const kel = window._kelompokList || [];
  const filtered = sel
    ? kel.filter((k) => k.kegiatan_nama === sel || k.tipe === sel)
    : [];
...
async function simpanJadwal() {
  ...
  if (semuaHari) {
    ...
    document
      .querySelectorAll("input[name=jHariCb]:checked")
      .forEach((cb) => hariList.push(cb.value));
  }
  if (!hariList.length) return toast("Pilih minimal 1 hari");
  const b = {
    hari_list: hariList,
    jam_mulai: $("jMulai").value,
    jam_selesai: $("jSelesai").value,
    kelompok_id: parseInt($("jKelompok").value),
    ustadz_username: $("jUstadz").value,
  };
...
async function hapusJadwal(id) {
  ...
  await api("/api/jadwal-umum/" + id, { method: "DELETE" });
  toast("Dihapus");
  loadJadwalUmum();
}
...
// -- JADWAL SEKOLAH MASTER --
async function loadJadwalSekolahMaster() {
  const kelas = await api("/api/kelas-sekolah");
  $("main").innerHTML =
    `<div class="page-header"><h2><i class="ri-calendar-blank-line"></i> Jadwal Pelajaran Sekolah (${kelas.length} Kelas)</h2></div>
    <div class="list-view">${
      kelas
        .map(
          (
            k,
          ) => `<div class="list-item au" onclick="openJadwalSekolah(${k.id},'${k.nama.replace(/'/g, "\\'")}')">
      <div class="left"><div class="icon" style="background:#eef2ff;color:#4f46e5"><i class="ri-calendar-line"></i></div>
      <div class="info"><div class="title">Jadwal Kelas ${k.nama}</div><div class="sub">Klik untuk kelola jadwal</div></div></div>
      <div class="actions"><i class="ri-arrow-right-s-line" style="color:var(--t3);font-size:1.5rem"></i></div></div>`,
        )
        .join("") || '<p style="color:var(--t3)">Belum ada kelas</p>'
    }
    </div>`;
}
async function openJadwalSekolah(kelasId, kelasNama) {
  kelasId = parseInt(kelasId);
  $("main").innerHTML =
    '<div class="card"><p style="color:var(--t3)">Loading...</p></div>';
  try {
    var kne = kelasNama.replace(/'/g, "\\'");
    const [jadwal, users] = await Promise.all([
      api("/api/jadwal-sekolah?kelas_id=" + kelasId).catch(() => []),
      api("/api/users").catch(() => []),
    ]);
    const kelasJadwal = jadwal || [];
    var isAdmin = ["admin", "superadmin"].includes(user.role);
    const ustadzList = users.filter(function (u) {
      return u.role === "ustadz";
    });
    var allHari = ["Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];
    var hariCbs = allHari
      .map(function (h) {
        return (
          "<label style=\"display:flex;align-items:center;gap:.25rem;padding:.35rem .55rem;background:#f8fafc;border:1.5px solid var(--border);border-radius:10px;cursor:pointer;font-size:.76rem;font-weight:500;transition:.2s\" onclick=\"setTimeout(function(){var c=this.querySelector('input');this.style.background=c.checked?'var(--greenbg)':'#f8fafc';this.style.borderColor=c.checked?'var(--green)':'var(--border)'}.bind(this),10)\"><input type=\"checkbox\" name=\"jsHariCb\" value=\"" +
          h +
    ...
    var html =
      '<button class="back-btn" onclick="loadJadwalSekolahMaster()"><i class="ri-arrow-left-line"></i> Kembali</button>';
    html +=
      '<div class="page-header"><h2><i class="ri-calendar-schedule-line"></i> Jadwal Kelas ' +
      kelasNama +
      "</h2></div>";

    html +=
      '<div class="card au"><h3><i class="ri-calendar-event-line"></i> Kelola Jadwal</h3>';
    ...
    if (isAdmin) {
      ...
      html +=
        '<div class="fg" style="flex:2;min-width:120px"><label>Mata Pelajaran</label><input id="jsMapel" placeholder="Matematika"></div>';
      html +=
        '<div class="fg" style="flex:2;min-width:100px"><label>Ustadz/Guru</label>' +
        searchSelect({
          id: "jsUstadz",
          items: ustadzList.map(function (u) {
            return { value: u.username, label: u.nama };
          }),
          placeholder: "Ketik nama ustadz...",
        }) +
        "</div>";
      html +=
        '<div class="fg" style="flex:3;min-width:100%"><label>Hari (centang beberapa)</label><div style="display:flex;gap:.35rem;flex-wrap:wrap">' +
        hariCbs +
        "</div></div>";
      html +=
        '<div style="display:flex;gap:.5rem;flex:2;min-width:100%"><div class="fg" style="flex:1"><label>Mulai</label><input type="time" id="jsMulai" value="07:00"></div><div class="fg" style="flex:1"><label>Selesai</label><input type="time" id="jsSelesai" value="08:00"></div>';
      html +=
        '<button class="btn btn-primary btn-sm" style="align-self:end;margin-bottom:.8rem" onclick="tambahJadwalSekolah(' +
        kelasId +
        ",'" +
        kne +
        '\')"><i class="ri-add-line"></i> Tambah</button></div></div>';
    }
    html +=
      '<div class="table-wrap"><table><tr><th>Hari</th><th>Pelajaran</th><th>Guru</th><th>Jam</th>' +
      (isAdmin ? "<th></th>" : "") +
      "</tr>";
    kelasJadwal
      .sort(function (a, b) {
        var d = ["Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];
        return (
          d.indexOf(a.hari) - d.indexOf(b.hari) ||
      ...
      .forEach(function (j) {
        html +=
          ...
          "</td>" +
          (isAdmin
            ? '<td><button class="btn btn-danger btn-sm" onclick="hapusJadwalSekolah(' +
              j.id +
              "," +
              kelasId +
              ",'" +
    ...
    if (!kelasJadwal.length)
      html +=
        '<tr><td colspan="5" style="text-align:center;color:var(--t3)">Belum ada jadwal</td></tr>';
    html += "</table></div></div>";

    $("main").innerHTML = html;
  } catch (e) {
    $("main").innerHTML =
      '<button class="back-btn" onclick="loadJadwalSekolahMaster()"><i class="ri-arrow-left-line"></i> Kembali</button><div class="card"><p style="color:var(--red)">Error: ' +
      e.message +
      "</p></div>";
  }
}

// Redirect tambahJadwalSekolah to openJadwalSekolah instead of openKelasDetail
const oldTambahJadwalSekolah = tambahJadwalSekolah;
tambahJadwalSekolah = async function (kelasId, kelasNama) {
  await oldTambahJadwalSekolah(kelasId, kelasNama);
  openJadwalSekolah(kelasId, kelasNama);
};
const oldHapusJadwalSekolah = hapusJadwalSekolah;
hapusJadwalSekolah = async function (id, kelasId, kelasNama) {
  await oldHapusJadwalSekolah(id, kelasId, kelasNama);
  openJadwalSekolah(kelasId, kelasNama);
};
  ...
  $("main").innerHTML = `
    <div class="abs-wrapper fade-up">
      <div class="abs-wrapper-header">
        <h2><i class="ri-moon-line"></i> Absen Kamar</h2>
        <div class="fg" style="margin-bottom:0.5rem"><label>Tanggal</label><input type="date" id="absMalamTgl" value="${today}" onchange="if(_absMalamKamarId) openAbsenMalamKamar(_absMalamKamarId, window._absMalamKamarNama)"></div>
      </div>
      <div id="absMalamTabs">
        ${kamar.map(k => `<div class="abs-list-item" id="tab-kamar-${k.id}" onclick="openAbsenMalamKamar(${k.id},'${k.nama.replace(/'/g, "\\'")}')">
          <div style="display:flex;align-items:center;gap:.6rem"><i class="ri-hotel-bed-line" style="color:var(--p)"></i> ${k.nama}</div>
          <i class="ri-arrow-right-s-line"></i>
        </div>`).join('') || '<div style="padding:1.5rem;text-align:center;color:var(--t3)">Belum ada kamar</div>'}
      </div>
    </div>
    <div id="absMalamContent">
      <div style="text-align:center;padding:2rem 1rem;color:var(--t3)"><i class="ri-arrow-up-line" style="font-size:1.5rem;display:block;margin-bottom:.5rem"></i>Pilih kamar di atas untuk mulai absen</div>
    </div>
  `;
}
...
async function openAbsenMalamKamar(kamarId, kamarNama) {
  ...
  document
    .querySelectorAll("#absMalamTabs .abs-list-item")
  ...
  $("absMalamContent").innerHTML = `
    ...
    <div class="card au" style="padding:0;overflow:hidden;border-radius:16px"><div class="table-wrap" style="border:none;box-shadow:none;border-radius:0"><table style="table-layout:fixed">
      ...
      ${members
        .map((m) => {
          const st = sudah ? existingMap[m.id] || "H" : "H";
        ...
        <td style="text-align:center"><span class="abs-toggle abs-a${st === "A" ? " active" : ""}${dis}" onclick="setAbsMalam(${m.id},'A',this)">A</span></td></tr>`;
        })
        .join("")}
    ...
    ${
      sudah
        ...
        : '<button class="btn btn-primary btn-full" id="btnAbsMalam" onclick="simpanAbsenMalam(' +
          kamarId +
          ')" style="box-shadow:0 -4px 16px rgba(0,0,0,.15)"><i class="ri-save-line"></i> Simpan Absen Kamar</button>'
    }</div>`;
  window._absMalamData = {};
  members.forEach(
    (m) =>
      (window._absMalamData[m.id] = sudah ? existingMap[m.id] || "H" : "H"),
  );
}

function setAbsMalam(santriId, status, el) {
  window._absMalamData[santriId] = status;
...
async function simpanAbsenMalam(kamarId) {
  const tanggal =
    $("absMalamTgl")?.value ||
    new Date(Date.now() + 7 * 3600000).toISOString().slice(0, 10);
  const items = Object.entries(window._absMalamData || {}).map(([sid, st]) => ({
  ...
  try {
    const res = await api("/api/absen-malam/bulk", {
      method: "POST",
      body: JSON.stringify({ tanggal, kamar_id: kamarId, items }),
    });
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="ri-save-line"></i> Simpan Absen Kamar';
      btn.style.opacity = "1";
    }
    toast("Error: " + e.message);
  }
...
async function loadKelasSekolah() {
  api("/api/kelas-sekolah").then(kelas => {
    var isAdmin = ["admin", "superadmin"].includes(user.role);
    $("main").innerHTML = `
      <div class="page-header fade-up">
        <h2><i class="ri-school-line"></i> Kelola Kelas Formal</h2>
        ${isAdmin ? '<button class="btn btn-primary btn-sm" onclick="showAddKelas()"><i class="ri-add-line"></i> Tambah</button>' : ""}
      </div>
      <div class="grid">
        ${kelas.map(k => `
        <div class="grid-card fade-up" style="padding:1.2rem; cursor:pointer;" onclick="openKelasDetail(${k.id},'${k.nama.replace(/'/g, "\'")}')">
          <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:.8rem">
            <div class="modern-grid-icon"><i class="ri-school-line"></i></div>
            ${isAdmin ? `<button class="btn btn-outline btn-sm" style="border:none; background:transparent; padding:4px" onclick="event.stopPropagation();showEditKelas(${k.id},'${k.nama.replace(/'/g, "\'")}')" title="Edit"><i class="ri-edit-line"></i></button>` : ''}
          </div>
          ...
          <div style="font-size:.8rem; color:var(--t3); display:flex; gap:.6rem; align-items:center">
            <span><i class="ri-group-line"></i> ${k.jumlah_santri || 0} Santri</span>
          </div>
        </div>`).join('') || '<div style="grid-column:1/-1; text-align:center; padding:2rem; color:var(--t3)">Belum ada kelas</div>'}
      </div>
    `;
  });
}
...
async function saveAddKelas() {
  const n = $("mdNama").value.trim();
  if (!n) return toast("Nama wajib");
  await api("/api/kelas-sekolah", {
    method: "POST",
...
async function saveEditKelas(id) {
  const n = $("mdEditNama").value.trim();
  if (!n) return toast("Nama wajib");
  await api("/api/kelas-sekolah/" + id, {
    method: "PUT",
    body: JSON.stringify({ nama: n }),
  });
  hideModal();
  toast("[OK]  Kelas diperbarui");
  loadKelasSekolah();
}
async function hapusKelas(id) {
  if (!confirm("Hapus kelas ini?")) return;
  await api("/api/kelas-sekolah/" + id, { method: "DELETE" });
  loadKelasSekolah();
}
...
async function openKelasDetail(kelasId, kelasNama) {
  ...
  try {
    ...
    var html =
      '<button class="back-btn" onclick="loadKelasSekolah()"><i class="ri-arrow-left-line"></i> Kembali</button>';
    html +=
      '<div class="page-header"><h2><i class="ri-school-line"></i> ' +
      kelasNama +
      "</h2></div>";
    html +=
      '<div class="card au"><h3><i class="ri-group-line"></i> Anggota (' +
      members.length +
      ")</h3>";
    isAdmin = ['admin', 'superadmin'].includes(user.role);
    if (isAdmin)
      html +=
        '<button class="btn btn-primary btn-sm" style="margin-bottom:.6rem" onclick="showBulkAddSantri({title:\'Tambah Santri ke ' +
        kelasNama.replace(/'/g, "\\'") +
        "',apiUrl:'/api/kelas-sekolah/" +
        kelasId +
        "/members/bulk',existingIds:[" +
        members
          .map(function (m) {
            return m.id;
          })
          .join(",") +
        "],onDone:function(){openKelasDetail(" +
        kelasId +
        ",'" +
        kne +
        '\');}})"><i class="ri-user-add-line"></i> Tambah Santri</button>';
    html +=
      '<div class="table-wrap"><table><tr><th>Nama</th><th>Kelas Diniyyah</th>' +
      (isAdmin ? "<th></th>" : "") +
      "</tr>";
    members.forEach(function (m) {
      html +=
        ...
        "</td>" +
        (isAdmin
          ? '<td><button class="btn btn-danger btn-sm" onclick="removeKelasMember(' +
            kelasId +
            "," +
            m.id +
            ",'" +
    ...
    if (!members.length)
      html +=
        '<tr><td colspan="3" style="text-align:center;color:var(--t3)">Belum ada anggota</td></tr>';
    html += "</table></div></div>";

    if (canNilai) {
      html +=
        '<div class="card au"><h3><i class="ri-book-open-line"></i> Mata Pelajaran Nilai (' +
        mapelSekolah.length +
        ")</h3>";
      isAdmin = ['admin', 'superadmin'].includes(user.role);
    if (isAdmin)
        html +=
          '<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.8rem;align-items:end"><div class="fg" style="flex:2;min-width:150px"><label>Nama Mapel</label><input id="mpsNama" placeholder="Matematika, IPA..."></div><button class="btn btn-primary btn-sm" style="margin-bottom:.8rem" onclick="tambahMapelSekolah(' +
          kelasId +
          ",'" +
          kne +
          '\')"><i class="ri-add-line"></i> Tambah</button></div>';
      html +=
        '<div class="table-wrap"><table><tr><th>Nama</th>' +
        (isAdmin ? "<th></th>" : "") +
        "</tr>";
      mapelSekolah.forEach(function (m) {
        html +=
          "<tr><td>" +
          m.nama +
          "</td>" +
          (isAdmin
            ? '<td><button class="btn btn-danger btn-sm" onclick="hapusMapelSekolah(' +
              m.id +
              "," +
              kelasId +
              ",'" +
      ...
      if (!mapelSekolah.length)
        html +=
          '<tr><td colspan="2" style="text-align:center;color:var(--t3)">Belum ada mapel nilai</td></tr>';
      html += "</table></div>";
      html += "</div>";
    }
    $("main").innerHTML = html;
    ...
    $("main").innerHTML =
      '<button class="back-btn" onclick="loadKelasSekolah()"><i class="ri-arrow-left-line"></i> Kembali</button><div class="card"><p style="color:var(--red)">Error: ' +
      e.message +
      "</p></div>";
  }
}
function searchKelasAdd(q, kelasId) {
  kelasId = parseInt(kelasId);
  const box = $("ksMbResults");
  if (!q) {
    box.innerHTML = "";
    return;
  }
  const r = (window._kelasAllSantri || [])
    .filter((s) => s.nama.toLowerCase().includes(q.toLowerCase()))
    .slice(0, 6);
  box.innerHTML = r
    .map(
      (s) =>
        `<div onclick="addKelasMember(${kelasId},${s.id})">${s.nama}</div>`,
    )
    .join("");
}
async function addKelasMember(kelasId, santriId) {
  ...
  try {
    await api("/api/kelas-sekolah/" + kelasId + "/members", {
      method: "POST",
      body: JSON.stringify({ santri_id: santriId }),
    });
    toast("[OK]  Ditambahkan");
    const kelas = await api("/api/kelas-sekolah");
    const k = kelas.find((x) => x.id === kelasId);
    openKelasDetail(kelasId, k?.nama || "Kelas");
  } catch (e) {
    toast("Error: " + e.message);
  }
}
async function removeKelasMember(kelasId, santriId, kelasNama) {
  ...
  try {
    await api("/api/kelas-sekolah/" + kelasId + "/members/" + santriId, {
      method: "DELETE",
    });
    toast("Dihapus");
    openKelasDetail(kelasId, kelasNama);
  } catch (e) {
    toast("Error: " + e.message);
  }
}
async function tambahJadwalSekolah(kelasId, kelasNama) {
  kelasId = parseInt(kelasId);
  const mp = $("jsMapel").value.trim(),
    u = $("jsUstadz_val") ? $("jsUstadz_val").value : $("jsUstadz").value,
    jm = $("jsMulai").value,
    js = $("jsSelesai").value;
  const hariList = [
    ...document.querySelectorAll("input[name=jsHariCb]:checked"),
  ].map((cb) => cb.value);
  if (!mp) return toast("Mata pelajaran wajib");
  if (!hariList.length) return toast("Pilih minimal 1 hari");
  try {
    await api("/api/jadwal-sekolah", {
      method: "POST",
      body: JSON.stringify({
        kelas_id: kelasId,
        mata_pelajaran: mp,
    ...
    toast("Jadwal ditambahkan untuk " + hariList.length + " hari");
    openKelasDetail(kelasId, kelasNama);
  } catch (e) {
    toast("Error: " + e.message);
  }
...
async function hapusJadwalSekolah(id, kelasId, kelasNama) {
  ...
  try {
    await api("/api/jadwal-sekolah/" + id, { method: "DELETE" });
    toast("Dihapus");
    openKelasDetail(kelasId, kelasNama);
  } catch (e) {
    toast("Error: " + e.message);
...
async function tambahMapelSekolah(kelasId, kelasNama) {
  const n = $("mpsNama")?.value?.trim();
  if (!n) return toast("Nama mapel wajib");
  try {
    await api("/api/mata-pelajaran-sekolah", {
      method: "POST",
      body: JSON.stringify({ kelas_id: kelasId, nama: n }),
    });
...
async function hapusMapelSekolah(mpId, kelasId, kelasNama) {
  if (!confirm("Hapus mata pelajaran ini? Semua nilai terkait akan terhapus."))
    return;
  try {
    await api("/api/mata-pelajaran-sekolah/" + mpId, { method: "DELETE" });
    toast("Dihapus");
    openKelasDetail(kelasId, kelasNama);
  } catch (e) {
    toast("Error: " + e.message);
...
async function loadAbsenSekolah() {
  ...
  api("/api/kelas-sekolah").then(kelas => {
    $("main").innerHTML = `
      <div class="abs-wrapper fade-up">
        <div class="abs-wrapper-header">
          <h2><i class="ri-school-line"></i> Absen Sekolah</h2>
          <div class="fg" style="margin-bottom:0.5rem"><label>Tanggal</label><input type="date" id="absSekolahTgl" value="${today}" onchange="if(_absSekolahState.kelasId) absSekolahPilihKelas(_absSekolahState.kelasId, _absSekolahState.kelasNama)"></div>
        </div>
        <div id="absSekolahTabs">
          ${kelas.map(k => `<div class="abs-list-item" id="tab-kelas-${k.id}" onclick="absSekolahPilihKelas(${k.id},'${k.nama.replace(/'/g, "\'")}')">
            <div style="display:flex;align-items:center;gap:.6rem"><i class="ri-school-line" style="color:var(--p)"></i> ${k.nama}</div>
            <i class="ri-arrow-right-s-line"></i>
          </div>`).join('') || '<div style="padding:1.5rem;text-align:center;color:var(--t3)">Belum ada kelas</div>'}
        </div>
      </div>
      <div id="absSekolahJadwalContainer" class="au"></div>
      <div id="absSekolahContent">
        <div style="text-align:center;padding:2rem 1rem;color:var(--t3)"><i class="ri-arrow-up-line" style="font-size:1.5rem;display:block;margin-bottom:.5rem"></i>Pilih kelas di atas untuk mulai absen</div>
      </div>
    `;
  });
}
...
async function absSekolahPilihKelas(kelasId, kelasNama) {
  ...
  document
    .querySelectorAll("#absSekolahTabs .abs-tab")
    .forEach((el) => el.classList.remove("active"));
  const tab = $("tab-kelas-" + kelasId);
  if (tab) tab.classList.add("active");

  $("absSekolahJadwalContainer").innerHTML =
    '<p style="text-align:center;color:var(--t3);font-size:.8rem;padding:.5rem"><i class="ri-loader-4-line ri-spin"></i> Memuat jadwal...</p>';
  ...
  $("absSekolahContent").innerHTML =
    '<div class="card au" style="text-align:center;padding:2rem 1rem;color:var(--t3)"><i class="ri-arrow-up-line" style="font-size:1.5rem;display:block;margin-bottom:.5rem"></i>Pilih mata pelajaran di atas</div>';
}
...
async function absSekolahPilihJadwal(jadwalId, mapel) {
  ...
  document
    .querySelectorAll("#absSekolahJadwalTabs .abs-tab")
    .forEach((el) => el.classList.remove("active"));
  ...
  $("absSekolahContent").innerHTML = `
    ...
    <div class="card au" style="padding:0;overflow:hidden;border-radius:16px"><div class="table-wrap" style="border:none;box-shadow:none;border-radius:0"><table style="table-layout:fixed">
      ...
      ${members
        ...
        <td style="text-align:center"><span class="abs-toggle abs-a${st === "A" ? " active" : ""}${dis}" onclick="setAbsSekolah(${m.id},'A',this)">A</span></td></tr>`;
        })
        .join("")}
    ...
    ${
      sudah
        ? '<button class="btn btn-full" disabled style="background:var(--green);color:#fff;opacity:.5"><i class="ri-checkbox-circle-line"></i> Sudah Diabsen</button>'
        : '<button class="btn btn-primary btn-full" id="btnAbsSekolah" onclick="simpanAbsenSekolah()" style="box-shadow:0 -4px 16px rgba(0,0,0,.15)"><i class="ri-save-line"></i> Simpan Absen Sekolah</button>'
    }</div>`;
  window._absSekolahData = {};
  members.forEach(
    (m) =>
      (window._absSekolahData[m.id] = sudah ? existingMap[m.id] || "H" : "H"),
  );
}

function setAbsSekolah(santriId, status, el) {
  window._absSekolahData[santriId] = status;
  const row = el.closest("tr");
  row
    .querySelectorAll(".abs-toggle")
    .forEach((s) => s.classList.remove("active"));
...
async function simpanAbsenSekolah() {
  ...
  const tanggal =
    $("absSekolahTgl")?.value ||
    new Date(Date.now() + 7 * 3600000).toISOString().slice(0, 10);
  const items = Object.entries(window._absSekolahData || {}).map(
    ([sid, st]) => ({ santri_id: parseInt(sid), status: st }),
  );
  if (!items.length) return toast("Tidak ada data");
  ...
  try {
    const res = await api("/api/absen-sekolah/bulk", {
      method: "POST",
      body: JSON.stringify({
        tanggal,
        kelas_id: kelasId,
        jadwal_sekolah_id: jadwalId,
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = '<i class="ri-save-line"></i> Simpan Absen Sekolah';
      btn.style.opacity = "1";
    }
    toast("Error: " + e.message);
  }
...
async function loadPembayaran() {
  try {
    ...
    let rekap = {};
    try {
      rekap = await api("/api/pembayaran/rekap?bulan=" + _pembayaranBulan);
    } catch (e) {
      rekap = {};
    }
    const persen = rekap.persen || 0;
    $("main").innerHTML =
      `<div class="page-header au"><h2><i class="ri-money-dollar-circle-line"></i> Pembayaran SPP</h2>
...
function pbTab(tab) {
  document.querySelectorAll("#pbTabs button").forEach((b) => {
  ...
  document.querySelectorAll("#pbTabs button").forEach((b) => {
    if (b.textContent.trim().includes(labels[tab] || "xxx"))
...
window.doExportSpp = function () {
  ...
  let url =
    "/api/pembayaran/export-excel?bulan_mulai=" + bm + "&bulan_akhir=" + ba;
  if (k) url += "&kamar_id=" + k;
  if (c) url += "&kelas_id=" + c;
  apiDownload(
    url,
    "Rekap_SPP_" + bm + (bm !== ba ? "_sd_" + ba : "") + ".xlsx",
  );
  hideModal();
};
...
async function showPembayaranList() {
  try {
    ...
    $("pbContent").innerHTML =
      `<div class="card au"><h3><i class="ri-list-check-2"></i> Status Pembayaran - ${_pembayaranBulan}</h3>
          ...
          s.status === "LUNAS"
            ...
            : s.status === "KURANG"
              ...
              : s.status === "GRATIS"
                ...
                : "badge-a";
        const st =
          s.status === "KURANG" ? "Kurang " + fmtRp(s.kekurangan) : s.status;
        const kat =
          s.kategori_nama && s.kategori_nama !== "Default"
            ? s.kategori_nama
            : "-";
        return (
          "<tr><td>" +
          ...
          "</td><td>" +
          (s.kamar || "-") +
          '</td><td><span style="font-size:.65rem;background:#f3f4f6;padding:.1rem .3rem;border-radius:4px">' +
          kat +
          '</span></td><td style="text-align:right">' +
          fmtRp(s.tarif) +
          '</td><td style="text-align:right;color:var(--red)">' +
          (s.potongan ? "-" + fmtRp(s.potongan) : "-") +
          '</td><td style="text-align:right;font-weight:700">' +
          fmtRp(s.tagihan) +
          '</td><td style="text-align:right">' +
          fmtRp(s.total_bayar) +
          '</td><td style="text-align:center"><span class="' +
          badge +
          '">' +
          st +
          "</span></td></tr>"
    ...
    $("pbContent").innerHTML =
      '<div class="card au" style="text-align:center;padding:2rem;color:var(--t3)">Error: ' +
      e.message +
      "</div>";
  }
}
...
async function showKategoriPanel() {
  try {
    ...
    $("pbContent").innerHTML =
      `<div class="card au"><h3><i class="ri-price-tag-3-line"></i> Kategori SPP Santri</h3>
...
async function simpanKategori() {
  try {
    const n = $("katNama").value.trim(),
      v = parseInt($("katNominal").value) || 0;
    if (!n) return toast("Nama wajib");
    await api("/api/pembayaran/kategori", {
      method: "POST",
      body: JSON.stringify({ nama: n, nominal: v }),
    });
...
async function hapusKategori(id) {
  if (!confirm("Hapus kategori ini? Santri yang memakainya akan direset."))
    return;
  try {
    await api("/api/pembayaran/kategori/" + id, { method: "DELETE" });
    toast("Dihapus");
    showKategoriPanel();
  } catch (e) {
    toast("Error: " + e.message);
...
async function showBulkSetKategori(kategoriId, nama) {
  ...
  function render(q) {
    const filt = q
      ? allSantri.filter(
          (s) =>
            ...
            (s.kamar_nama || "").toLowerCase().includes(q.toLowerCase()) ||
            (s.kelas_diniyyah || "").toLowerCase().includes(q.toLowerCase()) ||
            (s.kelas_sekolah || "").toLowerCase().includes(q.toLowerCase()),
        )
      : allSantri;
    var rows = "";
    filt.forEach(function (s) {
      ...
      var badge =
        s.kategori_spp_id === kategoriId
          ? ' <span style="font-size:.65rem;color:var(--green)">(Sudah)</span>'
          : "";
      var kelas = s.kelas_diniyyah
        ? '<span style="font-size:.65rem;background:#f3f4f6;padding:.1rem .3rem;border-radius:4px;margin-left:.3rem">' +
          s.kelas_diniyyah +
          "</span>"
        : "";
      rows +=
      ...
      rows +=
        '<input type="checkbox" ' +
        chk +
        ' onchange="this.checked?window._bulkKatSel.add(' +
        s.id +
        "):window._bulkKatSel.delete(" +
        s.id +
        ');document.getElementById(\'bulkCount\').textContent=window._bulkKatSel.size" style="accent-color:var(--green);width:16px;height:16px;flex-shrink:0">';
      rows +=
  ...
  var html =
    '<h3 style="margin-bottom:.8rem"><i class="ri-price-tag-3-line"></i> Tetapkan Kategori: ' +
    nama +
    "</h3>";
  html +=
  ...
  html +=
    '<div style="display:flex;gap:.5rem;margin-top:1rem;align-items:center"><button class="btn btn-primary" onclick="doBulkSetKategori(' +
    kategoriId +
    ')"><i class="ri-save-line"></i> Simpan (<span id="bulkCount">0</span> Santri)</button><button class="btn btn-outline" onclick="hideModal()">Batal</button></div>';
  $("modal").innerHTML = html;
  showModal();
...
async function doBulkSetKategori(kategoriId) {
  ...
  try {
    var r = await api("/api/santri/bulk-kategori", {
      method: "PUT",
      body: JSON.stringify({ santri_ids: ids, kategori_spp_id: kategoriId }),
    });
    hideModal();
    toast(r.message || "Kategori diterapkan ke " + ids.length + " santri");
    showKategoriPanel();
  } catch (e) {
    toast("Error: " + e.message);
  }
...
async function showTarifPanel() {
  try {
    ...
    $("pbContent").innerHTML =
      `<div class="card au"><h3><i class="ri-settings-3-line"></i> Rincian Tarif Syahriyah</h3>
...
async function simpanTarif() {
  try {
    const n = $("tarifNama").value.trim(),
      v = parseInt($("tarifNominal").value) || 0;
    if (!n || v <= 0) return toast("Nama dan nominal wajib");
    await api("/api/pembayaran/tarif", {
      method: "POST",
      body: JSON.stringify({ nama: n, nominal: v }),
    });
...
async function hapusTarif(id) {
  ...
  try {
    await api("/api/pembayaran/tarif/" + id, { method: "DELETE" });
    toast("Dihapus");
    showTarifPanel();
  } catch (e) {
    toast("Error: " + e.message);
...
async function showTarifPeriode(tarifId, nama) {
  try {
    const periodeList = await api(
      "/api/pembayaran/tarif/" + tarifId + "/periode",
    );
    ...
    let rows = arr
      .map(
        (p) =>
          ...
          '</td><td style="text-align:right;font-weight:700">' +
          fmtRp(p.nominal) +
          '</td><td><button class="btn btn-danger btn-sm" onclick="hapusTarifPeriode(' +
          p.id +
          "," +
          tarifId +
          ",'" +
    ...
    if (!arr.length)
      rows =
        '<tr><td colspan="4" style="text-align:center;color:var(--t3)">Belum ada periode khusus. Semua bulan pakai nominal default.</td></tr>';
    let html =
      '<h3 style="margin-bottom:.5rem"><i class="ri-calendar-line"></i> Periode Tarif: ' +
      nama +
      "</h3>";
    html +=
      '<p style="font-size:.78rem;color:var(--t3);margin-bottom:.8rem">Bulan yang tidak tercakup periode menggunakan nominal default. Periode tidak boleh tumpang tindih.</p>';
    html +=
      '<div class="table-wrap"><table><tr><th>Bulan Mulai</th><th>Bulan Akhir</th><th style="text-align:right">Nominal</th><th>Aksi</th></tr>' +
      rows +
      "</table></div>";
    html +=
      '<div style="margin-top:1rem;padding-top:.8rem;border-top:1px solid var(--border)">';
    html +=
      '<h4 style="font-size:.82rem;margin-bottom:.5rem"><i class="ri-add-circle-line"></i> Tambah Periode</h4>';
    html +=
      '<div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:end">';
    html +=
      '<div class="fg" style="flex:1;min-width:130px"><label>Bulan Mulai</label><input type="month" id="tpMulai"></div>';
    html +=
      '<div class="fg" style="flex:1;min-width:130px"><label>Bulan Akhir</label><input type="month" id="tpAkhir"></div>';
    html +=
      '<div class="fg" style="flex:1;min-width:120px"><label>Nominal (Rp)</label><input type="number" id="tpNom" placeholder="300000"></div>';
    html +=
      '<button class="btn btn-primary btn-sm" style="margin-bottom:.8rem" onclick="simpanTarifPeriode(' +
      tarifId +
      ",'" +
      nama.replace(/'/g, "\\'") +
      '\')"><i class="ri-add-line"></i> Simpan</button>';
    html += "</div></div>";
    html +=
      '<div style="margin-top:1rem"><button class="btn btn-outline btn-sm" onclick="hideModal()">Tutup</button></div>';
    $("modal").innerHTML = html;
    showModal();
  } catch (e) {
    toast("Error: " + e.message);
...
async function hapusTarifPeriode(periodeId, tarifId, nama) {
  ...
  try {
    await api("/api/pembayaran/tarif/periode/" + periodeId, {
      method: "DELETE",
    });
    toast("Dihapus");
    showTarifPeriode(tarifId, nama);
...
async function showKategoriPeriode(katId, nama) {
  try {
    const periodeList = await api(
      "/api/pembayaran/kategori/" + katId + "/periode",
    );
    const arr = Array.isArray(periodeList) ? periodeList : [];
    let rows = arr
      .map(
        (p) =>
          ...
          '</td><td style="text-align:right;font-weight:700">' +
          fmtRp(p.nominal) +
          '</td><td><button class="btn btn-danger btn-sm" onclick="hapusKategoriPeriode(' +
          p.id +
          "," +
          katId +
          ",'" +
    ...
    if (!arr.length)
      rows =
        '<tr><td colspan="4" style="text-align:center;color:var(--t3)">Belum ada periode khusus. Semua bulan pakai nominal default.</td></tr>';
    let html =
      '<h3 style="margin-bottom:.5rem"><i class="ri-calendar-line"></i> Periode Kategori: ' +
      nama +
      "</h3>";
    html +=
      '<p style="font-size:.78rem;color:var(--t3);margin-bottom:.8rem">Bulan yang tidak tercakup periode menggunakan nominal default. Periode tidak boleh tumpang tindih.</p>';
    ...
    html +=
      '<div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:end">';
    html +=
      '<div class="fg" style="flex:1;min-width:130px"><label>Bulan Mulai</label><input type="month" id="kpMulai"></div>';
    html +=
      '<div class="fg" style="flex:1;min-width:130px"><label>Bulan Akhir</label><input type="month" id="kpAkhir"></div>';
    html +=
      '<div class="fg" style="flex:1;min-width:120px"><label>Nominal (Rp)</label><input type="number" id="kpNom" placeholder="200000"></div>';
    html +=
      '<button class="btn btn-primary btn-sm" style="margin-bottom:.8rem" onclick="simpanKategoriPeriode(' +
      katId +
      ",'" +
      nama.replace(/'/g, "\\'") +
      '\')"><i class="ri-add-line"></i> Simpan</button>';
...
async function simpanKategoriPeriode(katId, nama) {
  try {
    const mulai = $("kpMulai").value,
      akhir = $("kpAkhir").value,
      nom = parseInt($("kpNom").value) || 0;
    if (!mulai || !akhir || nom <= 0) return toast("Semua field wajib diisi");
    if (mulai > akhir) return toast("Bulan mulai harus <= bulan akhir");
    await api("/api/pembayaran/kategori/" + katId + "/periode", {
      method: "POST",
      body: JSON.stringify({
        bulan_mulai: mulai,
        bulan_akhir: akhir,
    ...
    toast("Periode ditambahkan");
    showKategoriPeriode(katId, nama);
  } catch (e) {
    toast("Error: " + e.message);
  }
}
async function hapusKategoriPeriode(periodeId, katId, nama) {
  if (!confirm("Hapus periode ini?")) return;
  try {
    await api("/api/pembayaran/kategori/periode/" + periodeId, {
      method: "DELETE",
    });
    toast("Dihapus");
    showKategoriPeriode(katId, nama);
...
async function showPotonganPanel() {
  try {
    const [potongan, santri] = await Promise.all([
      api("/api/pembayaran/potongan"),
      api("/api/santri"),
    ]);
    ...
    const groups = {};
    arr.forEach((p) => {
      if (!groups[p.nama]) groups[p.nama] = { nominal: p.nominal, items: [] };
      groups[p.nama].items.push(p);
    });
    let gh = "";
    if (Object.keys(groups).length) {
      gh = Object.entries(groups)
        .map(function (e) {
          const nama = e[0],
            g = e[1];
          return (
            '<div style="margin-bottom:.5rem;background:var(--greenbg);border-radius:12px;padding:.6rem .8rem"><div style="display:flex;justify-content:space-between;margin-bottom:.3rem"><b style="font-size:.82rem">' +
            nama +
            '</b><b style="color:var(--green);font-size:.82rem">-' +
            fmtRp(g.nominal) +
            '</b></div><div style="display:flex;flex-wrap:wrap;gap:.3rem">' +
            g.items
              .map(function (p) {
                return (
                  '<span style="display:inline-flex;align-items:center;gap:.2rem;padding:.2rem .5rem;background:#fff;border-radius:6px;font-size:.72rem">' +
                  p.santri_nama +
                  ' <i class="ri-close-circle-line" style="cursor:pointer;color:var(--red)" onclick="hapusPotongan(' +
                  p.id +
                  ')"></i></span>'
                );
              })
      ...
      gh =
        '<div style="text-align:center;color:var(--t3);padding:1rem">Belum ada potongan</div>';
    }
    $("pbContent").innerHTML =
      '<div class="card au"><h3><i class="ri-scissors-cut-line"></i> Kelola Potongan</h3><p style="font-size:.78rem;color:var(--t3);margin-bottom:.8rem">Buat jenis potongan, lalu pilih santri penerima.</p>' +
      gh +
...
function searchPotSantri(q) {
  ...
  var r = (window._potSantriList || [])
    .filter(function (s) {
      return (
        s.nama.toLowerCase().indexOf(q.toLowerCase()) >= 0 &&
        !(window._potSelected || []).find(function (x) {
          return x.id === s.id;
        })
      );
    })
...
function rmPotS(id) {
  window._potSelected = window._potSelected.filter(function (x) {
    return x.id !== id;
  });
  renderPotSel();
}
function renderPotSel() {
  var el = $("potSel");
  if (!el) return;
  el.innerHTML = (window._potSelected || [])
    .map(function (s) {
      return (
        '<span style="display:inline-flex;align-items:center;gap:.3rem;padding:.25rem .6rem;background:var(--pbg);color:var(--p);border-radius:8px;font-size:.75rem;font-weight:600">' +
        s.nama +
        ' <i class="ri-close-line" style="cursor:pointer" onclick="rmPotS(' +
        s.id +
        ')"></i></span>'
      );
    })
...
async function simpanPotongan() {
  try {
    var ids = (window._potSelected || []).map(function (s) {
      return s.id;
    });
    var n = $("potNama").value.trim(),
      v = parseInt($("potNominal").value) || 0;
    if (!ids.length) return toast("Pilih santri dulu");
    if (!n || v <= 0) return toast("Jenis dan nominal wajib");
    await api("/api/pembayaran/potongan", {
      method: "POST",
      body: JSON.stringify({ santri_ids: ids, nama: n, nominal: v }),
    });
    toast("Potongan ditambahkan");
    showPotonganPanel();
  } catch (e) {
    toast("Error: " + e.message);
  }
}
async function hapusPotongan(id) {
  if (!confirm("Hapus?")) return;
  try {
    await api("/api/pembayaran/potongan/" + id, { method: "DELETE" });
    toast("Dihapus");
    showPotonganPanel();
  } catch (e) {
    toast("Error: " + e.message);
...
function searchBayarS(q) {
  ...
  var r = (window._bayarSantriList || [])
    .filter(function (s) {
      return s.nama.toLowerCase().indexOf(q.toLowerCase()) >= 0;
    })
    .slice(0, 8);
  box.innerHTML = r
    .map(function (s) {
      return (
        '<div onclick="pickBayarS(' +
        s.id +
        ",'" +
        s.nama.replace(/'/g, "\\'") +
        "')\">" +
...
async function pickBayarS(id, nama) {
  $("bayarSID").value = id;
  $("bayarSearch").value = nama;
  $("bayarSR").innerHTML = "";
  try {
    var bulan = $("bayarBulan").value || _pembayaranBulan;
    var data = await api("/api/pembayaran?bulan=" + bulan);
    var s = (Array.isArray(data) ? data : []).find(function (x) {
      return x.santri_id === id;
    });
    var box = $("bayarInfo");
    if (s) {
        ...
        s.status === "LUNAS"
          ...
          : s.status === "KURANG"
            ...
            : s.status === "GRATIS"
              ...
              : "badge-a";
      box.innerHTML =
        '<b><i class="ri-information-line"></i> Info Tagihan - ' +
        nama +
        '</b><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:.3rem;margin-top:.4rem;font-size:.78rem"><div>Tarif: <b>' +
        fmtRp(s.tarif) +
        '</b></div><div>Potongan: <b style="color:var(--red)">' +
        (s.potongan ? "-" + fmtRp(s.potongan) : "-") +
        '</b></div><div>Tagihan: <b style="color:var(--p)">' +
        fmtRp(s.tagihan) +
        "</b></div><div>Dibayar: <b>" +
        fmtRp(s.total_bayar) +
        '</b></div><div>Sisa: <b style="color:' +
        (s.kekurangan > 0 ? "var(--red)" : "var(--green)") +
        '">' +
        fmtRp(s.kekurangan) +
        '</b></div><div>Status: <span class="' +
        b +
        '">' +
        s.status +
        "</span></div></div>";
...
async function simpanBayar() {
  try {
    var sid = parseInt($("bayarSID").value);
    if (!sid) return toast("Pilih santri dulu");
    var bulan = $("bayarBulan").value,
      nom = parseInt($("bayarNom").value) || 0;
    if (!bulan || nom <= 0) return toast("Bulan dan nominal wajib");
    var r = await api("/api/pembayaran", {
      method: "POST",
      body: JSON.stringify({
        santri_id: sid,
        bulan: bulan,
        nominal: nom,
        metode: $("bayarMet").value,
        keterangan: $("bayarKet").value,
      }),
    });
...
async function refreshRekapStats() {
  try {
    ...
    // Update progress bar
    var lunas = document.querySelector(
      '.stat-card.c-blue ~ .stat-card ~ .stat-card ~ .card b[style*="green"]',
    );
    // Update the Lunas/Kurang/Belum row
    var spans = document.querySelectorAll(
      '.card [style*="justify-content:space-between"] b',
    );
    ...
    var bar = document.querySelector(
      '[style*="background:linear-gradient(90deg,#16a34a"]',
    );
    if (bar) bar.style.width = persen + "%";
  } catch (e) {
    console.error("Rekap refresh error:", e);
  }
}
...
async function showRiwayatPanel() {
  try {
    ...
    $("pbContent").innerHTML =
      '<div class="card au"><h3><i class="ri-history-line"></i> 200 Riwayat Pembayaran Terakhir</h3><div class="table-wrap"><table><tr><th>Tanggal Transaksi</th><th>Santri</th><th>Bulan SPP</th><th style="text-align:right">Nominal</th><th>Metode</th><th>Keterangan</th><th>Aksi</th></tr>' +
      arr
        .map(function (p) {
          var time = "00:00",
            date = "";
          ...
          return (
            '<tr><td style="font-size:.75rem;white-space:nowrap">' +
            date +
            " " +
            time +
            "</td><td>" +
            (p.santri_nama || "-") +
            '</td><td><span class="badge-h">' +
            (p.bulan || "-") +
            '</span></td><td style="text-align:right;font-weight:700">' +
            fmtRp(p.nominal) +
            '</td><td><span class="badge-s">' +
            (p.metode || "tunai") +
            '</span></td><td style="font-size:.78rem;color:var(--t3)">' +
            (p.keterangan || "-") +
            '</td><td><button class="btn btn-danger btn-sm" onclick="hapusBayar(' +
            p.id +
            ')"><i class="ri-delete-bin-line"></i></button></td></tr>'
          );
        })
      ...
      (arr.length
        ? ""
        : '<tr><td colspan="7" style="text-align:center;color:var(--t3)">Belum ada pembayaran</td></tr>') +
      "</table></div></div>";
  } catch (e) {
    $("pbContent").innerHTML =
      '<div class="card au" style="text-align:center;padding:2rem;color:var(--t3)">Error: ' +
...
async function hapusBayar(id) {
  if (!confirm("Hapus?")) return;
  try {
    await api("/api/pembayaran/" + id, { method: "DELETE" });
    toast("Dihapus");
    showRiwayatPanel();
    refreshRekapStats();
  } catch (e) {
...
async function loadCatatanBendahara() {
  try {
    ...
    var saldo = {};
    try {
      saldo = await api("/api/keuangan/saldo?" + qs);
    } catch (e) {
      saldo = {};
    }
    var data = [];
    try {
      data = await api("/api/keuangan?" + qs);
    } catch (e) {
      data = [];
    }
    var arr = Array.isArray(data) ? data : [];
    ...
    $("main").innerHTML =
      '<div class="page-header au"><h2><i class="ri-book-3-line"></i> Catatan Bendahara</h2></div>' +
      '<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.8rem;align-items:end">' +
      '<div class="fg" style="flex:1;min-width:130px;margin:0"><label>Dari</label><input type="date" id="keuTgl1" value="' +
      _keuTglMulai +
      '" onchange="_keuTglMulai=this.value;loadCatatanBendahara()"></div>' +
      '<div class="fg" style="flex:1;min-width:130px;margin:0"><label>Sampai</label><input type="date" id="keuTgl2" value="' +
      _keuTglAkhir +
      '" onchange="_keuTglAkhir=this.value;loadCatatanBendahara()"></div>' +
      ...
      '<div class="stat-card c-green au"><div class="stat-info"><div class="sn" style="font-size:.9rem">' +
      fmtRp(saldo.total_spp) +
      '</div><div class="sl">SPP</div></div><div class="si"><i class="ri-money-dollar-circle-line"></i></div></div>' +
      '<div class="stat-card c-teal au"><div class="stat-info"><div class="sn" style="font-size:.9rem">' +
      fmtRp(saldo.total_insidental) +
      '</div><div class="sl">Insidental</div></div><div class="si"><i class="ri-file-list-line"></i></div></div>' +
      '<div class="stat-card c-blue au"><div class="stat-info"><div class="sn" style="font-size:.9rem">' +
      fmtRp(saldo.total_masuk_lain) +
      '</div><div class="sl">Masuk Lain</div></div><div class="si"><i class="ri-add-circle-line"></i></div></div>' +
      '<div class="stat-card c-red au"><div class="stat-info"><div class="sn" style="font-size:.9rem">' +
      fmtRp(saldo.total_keluar) +
      '</div><div class="sl">Keluar</div></div><div class="si"><i class="ri-subtract-line"></i></div></div>' +
      '<div class="stat-card c-amber au"><div class="stat-info"><div class="sn" style="font-size:1.05rem">' +
      fmtRp(saldo.saldo) +
      '</div><div class="sl">SALDO</div></div><div class="si"><i class="ri-wallet-3-line"></i></div></div>' +
      "</div>" +
      ...
      '<div class="fg" style="flex:1;min-width:130px"><label>Tanggal</label><input type="date" id="keuTgl" value="' +
      ...
      arr
        .map(function (r) {
          ...
          return (
            '<tr><td style="font-size:.78rem;white-space:nowrap">' +
            (r.tanggal || "").slice(0, 10) +
            " " +
            time +
            "</td>" +
            '<td><span class="' +
            (isIn ? "badge-h" : "badge-a") +
            '">' +
            (isIn ? "MASUK" : "KELUAR") +
            "</span></td>" +
            '<td style="text-align:right;font-weight:700;color:' +
            (isIn ? "var(--green)" : "var(--red)") +
            '">' +
            (isIn ? "+" : "-") +
            fmtRp(r.nominal) +
            "</td>" +
            ...
            (r.keterangan || "-") +
            "</td>" +
            '<td><button class="btn btn-danger btn-sm" onclick="hapusCatatanKeu(' +
            r.id +
            ')"><i class="ri-delete-bin-line"></i></button></td></tr>'
          );
        })
      ...
      (arr.length
        ? ""
        : '<tr><td colspan="5" style="text-align:center;color:var(--t3)">Belum ada catatan</td></tr>') +
      "</table></div></div>";
  } catch (e) {
    toast("Error: " + e.message);
    console.error(e);
...
async function simpanCatatanKeu() {
  try {
    ...
    loadCatatanBendahara();
  } catch (e) {
    toast("Error: " + e.message);
  }
}
async function hapusCatatanKeu(id) {
  if (!confirm("Hapus catatan ini?")) return;
  try {
    await api("/api/keuangan/" + id, { method: "DELETE" });
    toast("Dihapus");
    loadCatatanBendahara();
  } catch (e) {
    toast("Error: " + e.message);
...
async function loadEPaket() {
  const [stats, santri] = await Promise.all([
    api("/api/paket/stats"),
    api("/api/santri"),
  ]);
  _epkSantriList = santri;
  const tabGerbang =
    _epkTab === "gerbang" ? "background:var(--p);color:#fff" : "";
  const tabAsrama =
    _epkTab === "asrama" ? "background:var(--p);color:#fff" : "";
  const tabLacak = _epkTab === "lacak" ? "background:var(--p);color:#fff" : "";
  $("main").innerHTML =
    `<div class="page-header fade-up"><h2><i class="ri-box-3-line"></i> E-Paket</h2></div>
    <div class="stat-grid fade-up" style="grid-template-columns:repeat(4,1fr);margin-bottom:1rem">
...
function epkSearchSantri(q) {
  ...
  const results = _epkSantriList
    .filter((s) => s.nama.toLowerCase().includes(q.toLowerCase()))
    .slice(0, 8);
  box.innerHTML = results
    .map(
      (s) =>
        `<div onclick="epkPickSantri(${s.id},'${s.nama.replace(/'/g, "\\'")}')"><strong>${s.nama}</strong> <span style="font-size:.72rem;color:var(--t3)">${s.kamar_nama || ""}</span></div>`,
    )
    .join("");
}
function epkPickSantri(id, nama) {
  $("epkSantri").value = id;
  $("epkSearch").value = nama;
  $("epkSearchResults").innerHTML = "";
  $("epkPengirim").focus();
}
...
function epkUpdateBulk() {
  _epkBulkIds = [...document.querySelectorAll(".epkCb:checked")].map((cb) =>
    parseInt(cb.value),
  );
  const cnt = $("epkBulkCount");
  if (cnt) cnt.textContent = _epkBulkIds.length + " dipilih";
  const btn = $("epkBulkBtn");
  if (btn) btn.disabled = _epkBulkIds.length === 0;
}
async function epkBulkTransfer() {
  ...
  const r = await api("/api/paket/bulk-transfer", {
    method: "PUT",
    body: JSON.stringify({ ids: _epkBulkIds, posisi_rak: rak }),
  });
  toast(r.message);
  _epkTab = "gerbang";
  loadEPaket();
...
async function hapusPaket(id) {
  ...
  await api("/api/paket/" + id, { method: "DELETE" });
  toast("Dihapus");
  loadEPaket();
}
    ...
    kamarKeys.forEach((kamar) => {
      ...
      tableHtml += `<div class="card fade-up" style="margin-bottom:.5rem;border-left:4px solid var(--blue)">
        ...
        <div class="table-wrap"><table class="epkAsramaTable">
          ...
          ${items
            .map(
              (p) => `<tr data-nama="${p.santri_nama.toLowerCase()}">
            <td><strong>${p.santri_nama}</strong></td>
            <td style="font-size:.78rem">${p.pengirim || "-"}</td>
...
function epkFilterAsrama(q) {
  ...
  rows.forEach((row) => {
    row.style.display =
      !q || row.dataset.nama.includes(q.toLowerCase()) ? "" : "none";
  });
}
...
async function loadEPaketLacak() {
  const statusOpts = [
    { v: "", l: "Semua" },
    { v: "DI_GERBANG", l: "Di Gerbang" },
    { v: "DI_ASRAMA", l: "Di Asrama" },
    { v: "SELESAI", l: "Selesai" },
  ];
  let url = "/api/paket";
  ...
  const statusBadge = (st) => {
    if (st === "DI_GERBANG")
      return '<span style="display:inline-block;padding:.2rem .5rem;border-radius:6px;font-size:.7rem;font-weight:600;background:#fef3c7;color:#d97706"><i class="ri-door-open-line"></i> Gerbang</span>';
    if (st === "DI_ASRAMA")
      return '<span style="display:inline-block;padding:.2rem .5rem;border-radius:6px;font-size:.7rem;font-weight:600;background:#dbeafe;color:#2563eb"><i class="ri-home-5-line"></i> Asrama</span>';
          ...
          paketList.length
            ? paketList
                .map(
                  (
                    p,
                  ) => `<tr data-search="${(p.santri_nama + " " + p.pengirim).toLowerCase()}">
          <td><strong>${p.santri_nama}</strong></td>
          <td style="font-size:.78rem">${p.kamar_nama || "-"}</td>
          <td style="font-size:.78rem">${p.pengirim || "-"}</td>
...
function epkFilterLacak(q) {
  ...
  rows.forEach((row) => {
    row.style.display =
      !q || row.dataset.search.includes(q.toLowerCase()) ? "" : "none";
  });
}
...
window.showBulkBayarModal = async function () {
  ...
  window._bulkBayarRender = function (q) {
    ...
    document.getElementById("bbInfo").innerHTML =
      `<span style="font-size:.78rem;color:var(--t3)">${filt.length} santri</span>`;
  };
...
window.doBulkBayar = async function () {
  ...
  if (
    !confirm(
      "Anda yakin ingin melunaskan pembayaran untuk santri & bulan yang dipilih?",
    )
  )
    return;
...
async function loadKegiatan() {
  const data = await api("/api/kegiatan");
  $("main").innerHTML = `
    <div class="abs-wrapper fade-up">
      <div class="abs-wrapper-header">
        <h2><i class="ri-movie-2-line"></i> Master Kegiatan</h2>
        <button class="btn btn-primary" onclick="kegiatanBaru()"><i class="ri-add-line"></i> Kegiatan Baru</button>
      </div>
      <div id="kegiatanTabs">
        ${data.map(k => `<div class="abs-list-item">
          <div style="display:flex;align-items:center;gap:.6rem"><i class="ri-movie-2-line" style="color:var(--p)"></i> ${k.nama} (${k.kategori || 'tambahan'})</div>
          <button class="btn btn-danger btn-sm" onclick="hapusKegiatan(${k.id})"><i class="ri-delete-bin-line"></i></button>
        </div>`).join('') || '<div style="padding:1.5rem;text-align:center;color:var(--t3)">Belum ada data</div>'}
      </div>
    </div>
  `;
}

async function kegiatanBaru() {
  const nm = prompt("Nama Kegiatan Baru:");
  if(!nm) return;
  const kat = prompt("Kategori (pokok/tambahan):", "tambahan");
  if(!kat) return;
  try {
    await api("/api/kegiatan", { method: "POST", body: JSON.stringify({ nama: nm, kategori: kat }) });
    toast("Berhasil");
    loadKegiatan();
  } catch(e) {
...
async function hapusKegiatan(id) {
  ...
  try {
    await api("/api/kegiatan/" + id, { method: "DELETE" });
    toast("Terhapus");
    loadKegiatan();
  } catch(e) {
    toast("Gagal: " + e.message);
...
async function loadKelompok() {
  const data = await api("/api/kelompok");
  $("main").innerHTML = `
    <div class="abs-wrapper fade-up">
      <div class="abs-wrapper-header">
        <h2><i class="ri-group-line"></i> Kelompok Kegiatan</h2>
        <button class="btn btn-primary" onclick="kelompokBaru()"><i class="ri-add-line"></i> Tambah</button>
      </div>
      <div id="kelompokTabs">
        ${data.map(k => `<div class="abs-list-item" onclick="openKelompokDetail(${k.id}, '${k.nama.replace(/'/g, "\\'")}')">
          <div style="display:flex;align-items:center;gap:.6rem"><i class="ri-group-line" style="color:var(--p)"></i> ${k.nama} (${k.kegiatan_nama || '-'})</div>
          <i class="ri-arrow-right-s-line"></i>
        </div>`).join('') || '<div style="padding:1.5rem;text-align:center;color:var(--t3)">Belum ada kelompok</div>'}
      </div>
    </div>
  `;
}

async function kelompokBaru() {
  const kgs = await api("/api/kegiatan");
  if(!kgs.length) return alert("Buat Master Kegiatan dulu!");
  let opt = kgs.map(x => `<option value="${x.id}">${x.nama}</option>`).join('');
  ...
  $("modal").innerHTML = `
    <div class="card fade-up">
      ...
      <div class="fg">
        ...
        <input type="text" id="kelNama">
      </div>
      <div class="fg">
        <label>Kegiatan Induk</label>
        <select id="kelKegiatanId">${opt}</select>
      </div>
      <div class="fg">
        <label>Tipe</label>
        <input type="text" id="kelTipe" value="ekstra" placeholder="Contoh: ekstra">
      </div>
      <div style="margin-top:1rem;display:flex;gap:.5rem;justify-content:flex-end">
        <button class="btn btn-outline" onclick="hideModal()">Batal</button>
        <button class="btn btn-primary" onclick="simpanKelompok()">Simpan</button>
      </div>
    </div>
  `;
  showModal();
...
async function simpanKelompok() {
  const n = $("kelNama").value;
  const t = $("kelTipe").value;
  const sel = $("kelKegiatanId");
  const kn = sel.options[sel.selectedIndex].text;
  const ki = parseInt(sel.value);
  
  if(!n) return toast("Nama wajib");
  try {
    await api("/api/kelompok", {
      method:"POST",
      body:JSON.stringify({nama:n, tipe:t, kegiatan_nama:kn, kegiatan_id:ki})
    });
    hideModal();
    toast("Tersimpan");
    loadKelompok();
...
async function openKelompokDetail(id, nama) {
  const mem = await api("/api/kelompok/" + id + "/members");
  $("main").innerHTML = `
    <div class="abs-wrapper fade-up">
      <div class="abs-wrapper-header">
        <div style="display:flex;align-items:center;gap:1rem">
          <button class="btn btn-outline btn-sm" onclick="loadKelompok()"><i class="ri-arrow-left-line"></i> Kembali</button>
          <h2 style="margin:0"><i class="ri-group-line"></i> ${nama}</h2>
        </div>
        <div style="display:flex;gap:.5rem">
          <button class="btn btn-primary" onclick="addKelompokMember(${id})"><i class="ri-user-add-line"></i> Tambah Anggota</button>
          <button class="btn btn-danger" onclick="hapusKelompok(${id})"><i class="ri-delete-bin-line"></i> Hapus Kelompok</button>
        </div>
      </div>
      <div style="padding:1rem">
        ${mem.map(m => `<div class="abs-list-item" style="cursor:default">
          <div style="display:flex;align-items:center;gap:.6rem"><i class="ri-user-3-line" style="color:var(--p)"></i> ${m.santri_nama}</div>
          <button class="btn btn-danger btn-sm" onclick="remKelompokMember(${id}, ${m.santri_id})"><i class="ri-close-line"></i></button>
        </div>`).join('') || '<div style="padding:1.5rem;text-align:center;color:var(--t3)">Belum ada anggota</div>'}
      </div>
    </div>
  `;
}
...
async function hapusKelompok(id) {
  if(!confirm("Yakin hapus kelompok ini beserta anggotanya?")) return;
  try {
    await api("/api/kelompok/" + id, {method:"DELETE"});
    toast("Terhapus");
    loadKelompok();
  } catch(e) { toast("Gagal: " + e.message); }
}

async function addKelompokMember(id) {
  const santri = await api("/api/santri");
  let opt = santri.map(s => `<option value="${s.id}">${s.nama}</option>`).join('');
  $("modal").innerHTML = `
    <div class="card fade-up">
      <h3>Tambah Anggota</h3>
      <div class="fg">
        <label>Pilih Santri</label>
        <select id="kelSantriId" multiple style="height:200px">${opt}</select>
        <small style="color:var(--t3)">Tahan Ctrl/Cmd untuk pilih banyak</small>
      </div>
      <div style="margin-top:1rem;display:flex;gap:.5rem;justify-content:flex-end">
        <button class="btn btn-outline" onclick="hideModal()">Batal</button>
        <button class="btn btn-primary" onclick="simpanKelompokMember(${id})">Simpan</button>
      </div>
    </div>
  `;
  showModal();
...
async function simpanKelompokMember(kelId) {
  const sel = $("kelSantriId");
  let ids = [];
  for(let o of sel.options) {
    if(o.selected) ids.push(parseInt(o.value));
  }
  if(!ids.length) return toast("Pilih minimal 1 santri");
  
  try {
    await api("/api/kelompok/" + kelId + "/members/bulk", {
      method:"POST",
      body:JSON.stringify({santri_ids: ids})
    });
    hideModal();
    toast("Berhasil");
    openKelompokDetail(kelId, "Loading...");
    setTimeout(() => loadKelompok(), 500); // Trigger reload to update names properly
  } catch(e) { toast("Gagal: " + e.message); }
}

async function remKelompokMember(kelId, santriId) {
  if(!confirm("Keluarkan santri dari kelompok?")) return;
  try {
    await api("/api/kelompok/" + kelId + "/members/" + santriId, {method:"DELETE"});
    openKelompokDetail(kelId, "Loading...");
    setTimeout(() => loadKelompok(), 500);
  } catch(e) { toast("Gagal: " + e.message); }
}

window.loadPengaturanRaportView = async function(kategori) {
  const settings = await api("/api/settings").catch(() => ({}));
  const isSuperadmin = ['admin', 'superadmin'].includes(user.role);
  let title = kategori === 'sekolah' ? 'Sekolah Formal' : (kategori === 'diniyyah' ? 'Madrasah Diniyyah' : 'Kegiatan');

  let html = `<div class="page-header au">
    <h2><i class="ri-settings-3-line"></i> Pengaturan Raport ${title}</h2>
  </div>`;
  
  if (!isSuperadmin) {
    html += `<div class="card"><p style="color:var(--red)">Anda tidak memiliki akses ke halaman ini.</p></div>`;
    $('main').innerHTML = html;
    return;
  }

  let kompKey = kategori === 'sekolah' ? 'komponen_nilai_sekolah' : 'komponen_nilai_diniyyah';
  let kompStr = settings[kompKey] || '[]';
  let kompData = [];
  try { kompData = JSON.parse(kompStr); } catch(e){}
  ...
  window._kompData = kompData;
  window._kompKategori = kategori;
  
  let kompHtml = `<div class="card au" style="margin-bottom:1rem">
    <h3 style="margin-bottom:1rem"><i class="ri-list-check-2"></i> Komponen Nilai</h3>
    <p style="font-size:0.85rem;color:var(--t2);margin-bottom:1rem">Atur sub-penilaian dan bobot (persentase) untuk perhitungan nilai akhir otomatis.</p>
    
    <div id="kompList"></div>
    <div style="margin-top:1rem;display:flex;gap:0.5rem">
      <button class="btn btn-outline btn-sm" onclick="addKompItem()"><i class="ri-add-line"></i> Tambah Komponen</button>
      <button class="btn btn-primary btn-sm" onclick="saveKomp()"><i class="ri-save-line"></i> Simpan Komponen</button>
    </div>
  </div>`;
...
window.renderKompList = function() {
  let list = document.getElementById('kompList');
  if(!list) return;
  let h = '';
  window._kompData.forEach((k, i) => {
    h += `<div style="display:flex;gap:0.5rem;margin-bottom:0.5rem;align-items:center">
      <input type="text" value="${k.nama}" id="komp_nama_${i}" placeholder="Nama (cth: Harian)" style="flex:1;padding:0.4rem;border:1px solid var(--border);border-radius:4px" />
      <input type="number" value="${k.bobot}" id="komp_bobot_${i}" placeholder="Bobot (%)" style="width:100px;padding:0.4rem;border:1px solid var(--border);border-radius:4px" />
      <button class="btn btn-danger btn-sm" onclick="delKompItem(${i})"><i class="ri-delete-bin-line"></i></button>
    </div>`;
  });
  list.innerHTML = h;
};

window.addKompItem = function() {
  window._kompData.push({nama:'', bobot:0});
  renderKompList();
};

window.delKompItem = function(idx) {
  window._kompData.splice(idx, 1);
  renderKompList();
};

window.saveKomp = async function() {
  for (let i = 0; i < window._kompData.length; i++) {
    window._kompData[i].nama = document.getElementById(`komp_nama_${i}`).value;
    window._kompData[i].bobot = parseFloat(document.getElementById(`komp_bobot_${i}`).value) || 0;
  }
  
  let key = window._kompKategori === 'sekolah' ? 'komponen_nilai_sekolah' : 'komponen_nilai_diniyyah';
  let payload = {};
  payload[key] = JSON.stringify(window._kompData);
  
  try {
    await api('/api/settings', { method: 'POST', body: JSON.stringify(payload) });
    toast('Berhasil menyimpan komponen nilai');
  } catch(e) {
    toast('Error: ' + e.message);
  }
};
...
async function loadJadwalDiniyyahMaster() {
  ...
  $("main").innerHTML =
    `<div class="page-header"><h2><i class="ri-calendar-blank-line"></i> Jadwal Pelajaran Diniyyah (${kelas.length} Kelas)</h2></div>
    <div class="list-view">${
      kelas
        .map(
          (
            k,
          ) => `<div class="list-item au" onclick="openJadwalDiniyyah(${k.id},'${k.nama.replace(/'/g, "\\'")}')">
      <div class="left"><div class="icon" style="background:#eef2ff;color:#4f46e5"><i class="ri-calendar-line"></i></div>
      <div class="info"><div class="title">Jadwal Kelas ${k.nama}</div><div class="sub">Klik untuk kelola jadwal</div></div></div>
      <div class="actions"><i class="ri-arrow-right-s-line" style="color:var(--t3);font-size:1.5rem"></i></div></div>`,
        )
...
async function openJadwalDiniyyah(kelasId, kelasNama) {
  kelasId = parseInt(kelasId);
  $("main").innerHTML =
    '<div class="card"><p style="color:var(--t3)">Loading...</p></div>';
  try {
    var kne = kelasNama.replace(/'/g, "\\'");
    const [jadwal, users] = await Promise.all([
      api("/api/jadwal-diniyyah?kelas_id=" + kelasId).catch(() => []),
      api("/api/users").catch(() => []),
    ]);
    const kelasJadwal = jadwal || [];
    var isAdmin = ["admin", "superadmin"].includes(user.role);
    ...
    var hariCbs = allHari
      .map(function (h) {
        return (
          "<label style=\"display:flex;align-items:center;gap:.25rem;padding:.35rem .55rem;background:#f8fafc;border:1.5px solid var(--border);border-radius:10px;cursor:pointer;font-size:.76rem;font-weight:500;transition:.2s\" onclick=\"setTimeout(function(){var c=this.querySelector('input');this.style.background=c.checked?'var(--greenbg)':'#f8fafc';this.style.borderColor=c.checked?'var(--green)':'var(--border)'}.bind(this),10)\"><input type=\"checkbox\" name=\"jdHariCb\" value=\"" +
          h +
          '" style="accent-color:var(--green);width:14px;height:14px"> ' +
          h +
          "</label>"
    ...
    var html =
      '<button class="back-btn" onclick="loadJadwalDiniyyahMaster()"><i class="ri-arrow-left-line"></i> Kembali</button>';
    html +=
      '<div class="page-header"><h2><i class="ri-calendar-schedule-line"></i> Jadwal Kelas ' +
      kelasNama +
      "</h2></div>";
    ...
    if (isAdmin) {
      html +=
        '<div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:.8rem;align-items:end">';
      html +=
        '<div class="fg" style="flex:2;min-width:120px"><label>Mata Pelajaran</label><input id="jdMapel" placeholder="Matematika"></div>';
      html +=
        '<div class="fg" style="flex:2;min-width:100px"><label>Ustadz/Guru</label>' +
        searchSelect({
          id: "jdUstadz",
      ...
      html +=
        ...
        "</div></div>";
      html +=
        '<div style="display:flex;gap:.5rem;flex:2;min-width:100%"><div class="fg" style="flex:1"><label>Mulai</label><input type="time" id="jdMulai" value="07:00"></div><div class="fg" style="flex:1"><label>Selesai</label><input type="time" id="jdSelesai" value="08:00"></div>';
      html +=
        '<button class="btn btn-primary btn-sm" style="align-self:end;margin-bottom:.8rem" onclick="tambahJadwalDiniyyah(' +
        kelasId +
        ",'" +
        kne +
        '\')"><i class="ri-add-line"></i> Tambah</button></div></div>';
    ...
    kelasJadwal
      ...
      .forEach(function (j) {
        html +=
          ...
          "</td>" +
          (isAdmin
            ? '<td><button class="btn btn-danger btn-sm" onclick="hapusJadwalDiniyyah(' +
              j.id +
              "," +
              kelasId +
              ",'" +
    ...
    $("main").innerHTML =
      '<button class="back-btn" onclick="loadJadwalDiniyyahMaster()"><i class="ri-arrow-left-line"></i> Kembali</button><div class="card"><p style="color:var(--red)">Error: ' +
      e.message +
      "</p></div>";
  }
}

// Redirect tambahJadwalDiniyyah to openJadwalDiniyyah instead of openKelasDetail

async function tambahJadwalDiniyyah(kelasId, kelasNama) {
  kelasId = parseInt(kelasId);
  const mp = $("jdMapel").value.trim(),
    u = $("jdUstadz_val") ? $("jdUstadz_val").value : $("jdUstadz").value,
    jm = $("jdMulai").value,
    js = $("jdSelesai").value;
  const hariList = [
    ...document.querySelectorAll("input[name=jdHariCb]:checked"),
  ].map((cb) => cb.value);
  if (!mp) return toast("Mata pelajaran wajib");
  if (!hariList.length) return toast("Pilih minimal 1 hari");
  try {
...
async function hapusJadwalDiniyyah(id, kelasId, kelasNama) {
  if (!confirm("Hapus jadwal ini?")) return;
  try {
    await api("/api/jadwal-diniyyah/" + id, { method: "DELETE" });
    toast("Dihapus");
    openKelasDetail(kelasId, kelasNama);
  } catch (e) {
    toast("Error: " + e.message);
