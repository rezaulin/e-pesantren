...
function showAddKeg(){
  $("modal").innerHTML=`<div class="card fade-up">
    <h3><i class="ri-movie-2-line"></i> Tambah Kegiatan Baru</h3>
    <div class="fg">
      <label>Nama Kegiatan</label>
      <input type="text" id="kegNama" placeholder="Contoh: Tahfidz, Karate, Marawis">
    </div>
    <div class="fg">
      <label>Kategori</label>
      <select id="kegKategori">
    ...
    <div style="display:flex;gap:.5rem;margin-top:1rem;justify-content:flex-end">
      ...
      <button class="btn btn-primary" onclick="simpanKegiatan()"><i class="ri-save-line"></i> Simpan</button>
    </div>
  </div>`;
  showModal();
  setTimeout(() => { const el = $("kegNama"); if(el) el.focus(); }, 200);
}
async function simpanKegiatan(){
  const nama = $("kegNama")?.value?.trim();
  const kategori = $("kegKategori")?.value || "tambahan";
  if(!nama) return toast("Nama kegiatan wajib diisi");
  try {
    await api("/api/kegiatan", {method:"POST", body:JSON.stringify({nama, kategori})});
    hideModal();
...
async function editKegiatan(id, namaLama, kategoriLama){
  $("modal").innerHTML=`<div class="card fade-up">
    <h3><i class="ri-edit-line"></i> Edit Kegiatan</h3>
    <div class="fg">
      <label>Nama Kegiatan</label>
      <input type="text" id="kegNamaEdit" value="${namaLama}">
    </div>
    <div class="fg">
      <label>Kategori</label>
      <select id="kegKategoriEdit">
        <option value="pokok" ${kategoriLama==="pokok"?"selected":""}>Pokok</option>
        <option value="tambahan" ${kategoriLama==="tambahan"?"selected":""}>Tambahan</option>
        <option value="ekstra" ${kategoriLama==="ekstra"?"selected":""}>Ekstra</option>
      </select>
    </div>
    <div style="display:flex;gap:.5rem;margin-top:1rem;justify-content:flex-end">
      <button class="btn btn-outline" onclick="hideModal()">Batal</button>
      <button class="btn btn-primary" onclick="updateKegiatan(${id})"><i class="ri-save-line"></i> Update</button>
    </div>
  </div>`;
  showModal();
}
async function updateKegiatan(id){
  const nama = $("kegNamaEdit")?.value?.trim();
  const kategori = $("kegKategoriEdit")?.value;
  if(!nama) return toast("Nama wajib diisi");
  try {
    await api("/api/kegiatan/"+id, {method:"PUT", body:JSON.stringify({nama, kategori})});
    hideModal();
    toast("Diperbarui");
    loadKegiatan();
  } catch(e) {
      ...
      <div class="grid">
        ...
        <div class="grid-card fade-up" style="padding:1.2rem; cursor:pointer;" onclick="openKelKegiatan(${k.id},'${k.nama.replace(/'/g, "\'")}')">
          <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:.8rem">
            ...
            ${isAdm ? `<button class="btn btn-outline btn-sm" style="border:none; background:transparent; padding:4px" onclick="event.stopPropagation();editKegiatan(${k.id},'${k.nama.replace(/'/g, "\'")}')" title="Edit"><i class="ri-edit-line"></i></button>` : ''}
          </div>
          <div style="font-weight:700; font-size:1.05rem; color:var(--text); margin-bottom:.3rem">${k.nama}</div>
          <div style="font-size:.8rem; color:var(--t3); display:flex; gap:.6rem; align-items:center">
